...
import { buildSpotlights } from "@/lib/v2/stats";
 ...
 */
export default function IdleSpotlight({
  matches,
  format,
  discipline,
}: {
  ...
  const stats = buildSpotlights(matches, format, discipline);
  ...
  return (
    <div className="rounded-2xl border border-court-line bg-court-panel px-[1.6vw] py-[1.8vh] flex flex-col overflow-hidden relative">
      <div
        ...
        className="v2-fade-up flex-1 min-h-0 flex flex-col justify-center"
      >
          {slide.kind === "stat" && (
            <>
          ...
          {slide.kind === "club" && (
            <div className="flex flex-col items-center text-center gap-[1.2vh]">
              <span className="v2-breathe opacity-80">
                <ClubMark size={54} />
              </span>
              <p
