...
import { ServeBadge, ServeHandover, useServeHandover } from "@/components/v3/ServeIndicator";
...
function CoachConsole({ courtId }: { courtId: number }) {
  ...
  const outbox = useOutbox({
    ...
    onAccepted: () => useV3Store.getState().refresh(),
  });
  ...
  function submitPoint(slot: 1 | 2, matchId: string) {
    const fromServer = useV3Store.getState().serverPointsFor(matchId);
  ...
  function scoreNow(slot: 1 | 2, target: MatchDTO) {
    ...
    useV3Store.getState().optimisticPoint(target.id, toDTO(newState, config));
  ...
  async function undo() {
    ...
    setBusy(true);
    const expectedLastSeq = useV3Store.getState().serverPointsFor(match.id);
  ...
  if (!Number.isInteger(courtId)) {
    return (
      <div className="min-h-screen flex items-center justify-center text-white/50">
        Bad court in the URL — try <span className="text-gold ml-1">/v3/coach/2</span>
      </div>
    );
  }
