...
function SideRow({ match, slot, leading, race }: { match: MatchDTO; slot: 1 | 2; leading: boolean; race: boolean }) {
  const i = slot - 1;
  ...
  return (
    <div className="h-screen w-screen overflow-hidden flex flex-col relative bg-court-bg">
      ...
      <main className="relative flex-1 min-h-0 flex flex-col justify-center">
        <SideRow match={match} slot={1} leading={lead1} race={race} />
        <div className="h-px bg-white/10 mx-[3vw]" />
        <SideRow match={match} slot={2} leading={lead2} race={race} />
      </main>
