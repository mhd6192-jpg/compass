...
import { placeMedal, placeTitle } from "@/components/v2/CeremonyScreen";
import { useV2Store } from "@/store/useV2Store";
import { usePinStore } from "@/store/usePinStore";
import { currentAward } from "@/lib/v2/stage";

type Action = "configure" | "start" | "next" | "back" | "reset";
...
function CeremonyRemote() {
  ...
  async function send(action: Action, places?: number[]) {
    setError(null);
    setBusy(true);
    try {
      const res = await fetch("/api/v2/ceremony", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action, places, pin }),
      });
    ...
    return (
      <main className="min-h-screen p-4 max-w-lg mx-auto pb-16">
        ...
        {revealOrder.length > 0 && (
          <p className="text-white/45 text-xs mb-5 text-center">
            Reveal order:{" "}
        ...
        <div className="flex items-center justify-center gap-4 mt-8">
          <Link href="/v2" className="text-white/35 text-xs underline underline-offset-4">
            v2 hub
          </Link>
  ...
  return (
    <main className="min-h-screen p-4 max-w-lg mx-auto flex flex-col pb-10">
      ...
      {!done ? (
        ...
        >
          {nextPlace !== undefined ? `Announce ${placeTitle(nextPlace).toLowerCase()}` : "Show the full podium"}
        </button>
      ) : (
        <button
          onClick={() => send("reset")}
