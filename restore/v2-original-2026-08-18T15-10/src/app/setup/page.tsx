...
import { isPointsRace, type TiebreakMode } from "@/lib/types";
...
export default function SetupPage() {
  ...
  const [format, setFormat] = useState<"compass" | "round-robin">("compass");
    ...
    <main className="min-h-screen p-4 sm:p-8 max-w-3xl mx-auto">
      ...
      <section className="mb-6">
        ...
        <div className="flex gap-2">
          <button
            type="button"
            onClick={() => setFormat("compass")}
            className={`flex-1 rounded-xl py-3 font-display uppercase text-sm border ${
              format === "compass" ? "bg-gold text-court-bg border-gold font-bold" : "border-court-line text-white/60"
            }`}
          >
            Compass (16, single elim)
          </button>
          <button
            type="button"
            onClick={() => setFormat("round-robin")}
            className={`flex-1 rounded-xl py-3 font-display uppercase text-sm border ${
              format === "round-robin" ? "bg-gold text-court-bg border-gold font-bold" : "border-court-line text-white/60"
            }`}
          >
            Round robin group
          </button>
      ...
      {format === "compass" ? (
        ...
        <section className="mb-6">
          ...
          <p className="text-white/40 text-xs mt-3">
            Every team plays every other team once ({rrNames.length} teams → {(rrNames.length * (rrNames.length - 1)) / 2} matches). Standings
            are ranked by matches won.
          </p>
        </section>
      )}
