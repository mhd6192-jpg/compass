...
import { isPointsRace } from "@/lib/types";
...
export async function POST(req: Request) {
  try {
    ...
    const fmt: "compass" | "round-robin" = format === "round-robin" ? "round-robin" : "compass";

    if (!Array.isArray(names) || (fmt === "compass" ? names.length !== 16 : names.length < 3)) {
      return NextResponse.json(
        { error: fmt === "compass" ? "Exactly 16 player names are required" : "At least 3 teams are required" },
        { status: 400 }
      );
    }
