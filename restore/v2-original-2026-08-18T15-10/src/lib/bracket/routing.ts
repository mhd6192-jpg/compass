...
async function completeMatch(tx: Tx, matchId: string, winnerSlot: 1 | 2, opts: CompleteOpts = {}) {
  ...
  // A round-robin has no championship match — except the play-off, when there is one.
  const championshipWon = (match.bracket === "E" && match.isBracketFinal) || isDeciderRow(match);

  return { championshipWon, affected: [...routedIds, ...(deciderId ? [deciderId] : []), ...courtChangedIds] };
}
...
export async function scorePoint(client: PrismaClient, matchId: string, slot: 1 | 2): Promise<ScorePointResult> {
  return client.$transaction(async (tx) => {
    ...
    await tx.pointEvent.create({ data: { matchId, seq, slot } });
...
export async function undoLastPoint(client: PrismaClient, matchId: string): Promise<{ affectedMatchIds: string[] }> {
  return client.$transaction(async (tx) => {
    const match = await tx.match.findUniqueOrThrow({ where: { id: matchId } });
    ...
    return { affectedMatchIds: Array.from(new Set(affected)) };
  });
}
...
export async function forceEndMatch(
  ...
  winnerSlot: 1 | 2,
  reason: string
): Promise<{ championshipWon: boolean; affectedMatchIds: string[] }> {
  return client.$transaction(async (tx) => {
    const result = await completeMatch(tx, matchId, winnerSlot, { forced: true, reason });
    return { championshipWon: result.championshipWon, affectedMatchIds: Array.from(new Set([matchId, ...result.affected])) };
  });
}
